...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      // If we are already on the login page, don't enforce restrictions here
  ...
  return (
    <div className={isLogin ? "min-h-screen bg-surface" : "bg-surface font-body-md text-on-surface antialiased min-h-screen"}>
      {/* SideNavBar */}
      {!isLogin && (
      ...
      <main className={isLogin ? "min-h-screen flex flex-col" : "ml-64 min-h-screen flex flex-col"}>
        {/* Admin Top Bar */}
        {!isLogin && (
